...
from sklearn.linear_model import LinearRegression as LReg # LinearRegression(positive=True) gives fast non-negative regression; nnls is the strict fallback
...
# ---------- Helpers ----------
"""
FreeSurfer's .annot files store label names as byte strings by default. Python 3 makes a strict distinction between bytes (binary data) and Unicode strings. Without
conversion, operations like string comparison or regex matching would fail. 
"""
...
def _ids_for(names_array, targets):
    """Return label indices whose normalized name is in targets (exact, case-insensitive)."""
...
def build_primary_masks_fs5(lh_annot_path, rh_annot_path):
    """
    Create boolean masks for V1, S1 (3a/3b/1/2), A1 on fsaverage5.
    Assumes vertex order is [LH (10,242); RH (10,242)].

    (1) Inputs:
        - lh_annot_path: Path to left hemisphere annotation file
        - rh_annot_path: Path to right hemisphere annotation file
    (2) Processing:
        - reads annotation files for both hemisphere
    """
...
def v_ts_nnls_vertex_fs5(ts, masks):
    ...
    ts_v1 = ts[masks["V1"], :].mean(axis=0)
    ts_s1 = ts[masks["S1"], :].mean(axis=0)
    ts_a1 = ts[masks["A1"], :].mean(axis=0)

    X = np.vstack((ts_v1, ts_s1, ts_a1)).T  # [T x 3]
    Y = ts.T                                 # [T x V]

    # fast path: sklearn positive regression (multi-output)
    coefs = np.zeros((3, V))
    ...
    try:
        ...
        # fallback: strict NNLS per vertex (slower)
        for v in range(V):
            b, _ = nnls(X, Y[:, v])
            coefs[:, v] = b
            Y_hat[:, v] = X @ b
    ...
    ss_total = np.where(ss_total == 0, np.finfo(float).eps, ss_total)
    rgba[3, :] = ss_exp / ss_total
    return rgba

# ---------- color models ----------
def v_rgba2hue(rgba):
    """Hue (degrees) from RGB betas (rows 0–2)."""
    rgbv = rgba[:3, :]
...
def v_hsv_model_rgba(rgba):
    ...
    denom = (rd.max() - rd.min())
    rd = (rd - rd.min()) / (denom if denom != 0 else 1)
...
def v_hsv_model_rgba_indiv(rgba):
    ...
    rd_group = (ex_group - ex_group.min()) / (denom if denom != 0 else 1)
